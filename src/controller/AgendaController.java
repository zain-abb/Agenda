...
public class AgendaController implements Initializable {
    ...
    private void fillGridPane() {
        //Placing 35 Panels in the GridPane
        for (int i = 0; i < 5; i++) {
            for (int j = 0; j < 7; j++) {
    ...
    private void updateCalendar(YearMonth yearMonth) {
        ...
        //Go to the last Monday of the previous month, if not the 1st of the month is Monday
        while (!localDate.getDayOfWeek().toString().equals("MONDAY")) {
            localDate = localDate.minusDays(1);
        }
        LocalDate today = LocalDate.now();
        //Filling the calendar with the days' numbers
        for (CalendarPaneModel pane : allCalendarDays) {
            Text text = new Text(10, 60, String.valueOf(localDate.getDayOfMonth()));
            //writing green today
            ...
            pane.setOnMouseClicked(e -> {
                try (PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("date.txt", false)))) {
                    out.println(lblMonth.getText() + " " + text.getText() + ", " + lblYear.getText());
                ...
                System.out.println(lblMonth.getText() + " " + text.getText() + ", " + lblYear.getText());
                dialog.show();
            });
